...
int main(int argc, char* argv[]) {
	...
	if(socket_desc<0) handle_error("Client main: socket() error");
	
	
	//inizializzo connessione lato client
	ret= connect(socket_desc, (struct sockaddr*) &server_addr, (socklen_t) sock_len);
     ...
     */
     ...
     if(ret<0) handle_error("Client main: close socket_desc error");
