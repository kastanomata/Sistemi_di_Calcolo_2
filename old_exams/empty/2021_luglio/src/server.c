...
int main(int argc, char *argv[])
{
     ...
     */
	 ...
	 ret= bind(listening_socket_desc, (struct sockaddr *) &server_addr, (socklen_t) sockaddr_len);
	  if(ret<0) handle_error("Server main: bind su socket error");
	 
	 
	 //listen
	 ret= listen(listening_socket_desc, MAX_CONN_QUEUE);
	 if(ret<0) handle_error("Server main: listen su socket error");
    ...
    while (1) {
         ...
         */
         pid=fork();
         if(pid==-1) handle_error("Server main: fork error");
         if(pid==0){//-----sono il processo figlio
			 printf("Hello sono [Child%d]\n",getpid());
			 
			 ret=close(listening_socket_desc);
			 if(ret<0) handle_error("Child: close listening_socket_desc error");
		...
		ret=close(client_desc);
		 if(ret<0) handle_error("Padre: close client_desc error");
     ...
     */
	ret= close(listening_socket_desc);
	 if(ret<0) handle_error("Padre: close listening_socket_desc error");
