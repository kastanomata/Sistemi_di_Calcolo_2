...
// semaphores
sem_t transaction_s;
sem_t stop_production_s;
sem_t start_reading_s;
...
void* performTransactions(void* x) {
    ...
    while (args->numOps > 0) {
        ...
        ret = sem_wait(&stop_production_s); // c'è spazio per produrre?
        if(ret == -1) handle_error("Error!");
        ret = sem_wait(&transaction_s); // qualcun altro sta accedendo al buffer?
        if(ret == -1) handle_error("Error!");
        // write the item and update write_index accordingly
        transactions[write_index] = currentTransaction;
        write_index = (write_index + 1) % BUFFER_SIZE;
        ret = sem_post(&start_reading_s); // segnalo che c'è qualcosa da leggere nel buffer
        if(ret == -1) handle_error("Error!");
        ret = sem_post(&transaction_s); // segnalo che non sto più accedendo al buffer
        if(ret == -1) handle_error("Error!");
        args->numOps--;
        //printf("P %d\n", args->numOps);
    }
...
void* processTransactions(void* x) {
    ...
    int ret;
    while (args->numOps > 0) {
        ret = sem_wait(&start_reading_s); // c'è qualcosa da leggere nel buffer?
        if(ret == -1) handle_error("Error!");
        ret = sem_wait(&transaction_s); // qualcun altro sta accedendo al buffer?
        if(ret == -1) handle_error("Error!");
        ...
        ret = sem_post(&stop_production_s); // segnalo che c'è spazio per produrre nel buffer
        if(ret == -1) handle_error("Error!");
        ret = sem_post(&transaction_s); // segnalo che non sto più accedendo al buffer
        if(ret == -1) handle_error("Error!");
        args->numOps--;
...
int main(int argc, char* argv[]) {
    ...
    ret = sem_init(&transaction_s, 0, 1);
    if(ret == -1) handle_error("Error!");
    ret = sem_init(&stop_production_s, 0, BUFFER_SIZE);
    if(ret == -1) handle_error("Error!");
    ret = sem_init(&start_reading_s, 0, 0);
    if(ret == -1) handle_error("Error!");
    ...
    srand(PRNG_SEED); 
    // NOTE: check that NUM_PRODUCERS, NUM_CONSUMERS is set according to the right spec
    ...
    ret = sem_destroy(&transaction_s);
    if(ret == -1) handle_error("Error!");
    ret = sem_destroy(&stop_production_s);
    if(ret == -1) handle_error("Error!");
    ret = sem_destroy(&start_reading_s);
    if(ret == -1) handle_error("Error!");
    exit(EXIT_SUCCESS);
}
